				...
				if (myjob.name == v.job or v.job == nil) and not v.used then
					...
					if distance <= Config.Range+2 then
						...
						if distance <= Config.Range then
							dormir = 1
							DrawText3D(v.coords.x, v.coords.y, v.coords.z, "~r~E~w~ - Modificar el vehículo")
...
-- Add helper to build chameleon submenu
function AddChameleonColors()
	local submenu = {}
	for i = 1, #Config.Chameleon do
		local ch = Config.Chameleon[i]
		submenu[#submenu+1] = {
			-- Changed: use sequential index instead of ch.id
			type = json.encode({tipo = "chameleon", index = i}),
			title = ch.label,
...
function ModsAvailable(carro)
	...
	-- Add new chameleon color to the paint submenu:
	mods[#mods+1] = AddToMenu("chameleon", AddChameleonColors(), 0)
	return mods
end
...
RegisterNUICallback("action", function(data)
	...
	if data.action == "openSubMenu" then -- aqui e cuando el jugador hace clic para abrir
	...
	elseif data.action == "subMenuAction" then  -- aquí es cuando el jugador hace clic para seleccionar el nivel de modificación
		local tab = json.decode(data.type)
		local mod = tab.tipo
		menuatual = mod
		...
		elseif mod == "chameleon" then
			-- Obtener datos RGB usando el índice correcto de Config.Chameleon
			local colorData = Config.Chameleon[tab.index] or {}  -- Se elimina el ajuste de índice
			if colorData.rgb then
				AddMoneyNotDefault(TunagensDefault["chameleon"] or 0, Config.TunningPrices["chameleon"] or {base=1000, bylevel=0}, tab.index, 0)
				TunagensDefault["chameleon"] = tab.index
				SetVehicleCustomPrimaryColour(carroselected, colorData.rgb.r, colorData.rgb.g, colorData.rgb.b)
				SetVehicleCustomSecondaryColour(carroselected, colorData.rgb.r, colorData.rgb.g, colorData.rgb.b)
			else
				ESX.ShowNotification("Color de camaleón no definido")
			end
		else
			AplicarMod(tab.tipo,tab.index-2)
		end
		PlaySoundFrontend(-1, "OK", "HUD_FRONTEND_DEFAULT_SOUNDSET", 1)
    elseif data.action == "opensubSubMenu" then  -- aquí es cuando el jugador hace clic para seleccionar el nivel de modificación
		local tab = json.decode(data.type)
		menuatual = tab.tipo
		camControl(tab.tipo)
		PlaySoundFrontend(-1, "OK", "HUD_FRONTEND_DEFAULT_SOUNDSET", 1)
    elseif data.action == "subSubMenuAction" then  -- aquí es cuando el jugador hace clic para seleccionar el nivel de modificación
		local tab = json.decode(data.type)
		...
		elseif tab.tipo == "chameleon" then
			AddMoneyNotDefault(TunagensDefault["chameleon"], Config.TunningPrices["chameleon"] or {base=1000,bylevel=0}, tab.index, GetVehicleColours(carroselected))
			TunagensDefault["chameleon"] = tab.index
			SetVehicleColours(carroselected, tab.index, tab.index)
		end
		PlaySoundFrontend(-1, "OK", "HUD_FRONTEND_DEFAULT_SOUNDSET", 1)
	elseif data.action == "backToMainMenu" then -- cuando haga clic en el botón para volver
	...
	elseif data.action == "cancel" then-- cuando termine
		...
		menuatual = nil
		carroselected = nil
    elseif data.action == "finish" then -- cuando termine
		-- Agregar lógica para enviar factura
		local amount = preco
		local closestPlayer, closestDistance = ESX.Game.GetClosestPlayer()
		if closestPlayer == -1 or closestDistance > 3.0 then
			ESX.ShowNotification("No hay jugadores cercanos. La factura no se enviará y el menú permanece abierto.")
			return  -- No se cierra el menú
		end
		local societyName = "society_" .. Config.Job
		TriggerServerEvent('esx_billing:sendBill', GetPlayerServerId(closestPlayer), societyName, "Tuning", amount)
		ESX.ShowNotification("Factura enviada por $"..amount)
		if preco > 0 then
		...
		menuatual = nil
		carroselected = nil
	elseif data.action == "kinda" then-- cuando termine
		PlaySoundFrontend(-1, "NO", "HUD_FRONTEND_DEFAULT_SOUNDSET", 1)
	elseif data.action == "camlivre" then-- cuando termine
		if focuson then
			camControl("close")
			SetNuiFocus(false, false)
			SendNUIMessage({
	...
	elseif data.action == "backTosubMainMenu" then -- cuando haga clic en el botón para volver
		PlaySoundFrontend(-1, "NO", "HUD_FRONTEND_DEFAULT_SOUNDSET", 1)
	end
end)
...
function CancelEverything(data)
	for k,v in pairs(TunagensDefault) do
		...
		elseif k == "chameleon" then
			local old = TunagensDefault["chameleon"]
			SetVehicleColours(carroselected, old, old)
		end
	end
end
